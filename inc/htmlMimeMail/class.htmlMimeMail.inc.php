...
require_once(dirname(__FILE__) . '/class.Mail_mimePart.inc.php');
...
//echo '['.$filename.':none]';
    ...
    */
    function _findHtmlImages($images_dir)
        ...
        for ($i=0; $i<count($images[1]); $i++)
            ...
            elseif (file_exists($images_dir . $images[1][$i]))
				{
                $html_images[] = $images_dir.$images[1][$i];
                $this->html = str_replace($images[1][$i], basename($images[1][$i]), $this->html);
	            }
	        }
        ...
        if (!empty($html_images))
            ...
            for ($i=0; $i<count($html_images); $i++)
				{
                if ($image = $this->getFile($html_images[$i]))
					{
    ...
    */
    function &_addAlternativePart(&$obj, $body='')
    {
        $params['content_type'] = 'multipart/alternative';
        if (is_object($obj)) {
            $return = &$obj->addSubpart($body, $params);
        } else {
            $return = new Mail_mimePart($body, $params);
        }
        return $return;
    }
    ...
    * Adds a html subpart to a mime_part object
    */
    function &_addRelatedPart(&$obj, $body='')
    {
    ...
    */
    function send($recipients, $type = 'mail')
    {
        ...
        switch ($type) {
            ...
            case 'smtp':
                require_once(dirname(__FILE__) . '/class.smtp.inc.php');
                require_once(dirname(__FILE__) . '/class.Mail_RFC822.inc.php');
                    ...
                    if ($name == 'Cc' OR $name == 'Bcc') {
						if (is_array($value))
							...
							foreach($value as $val)
								{
		                        $addresses = Mail_RFC822::parseAddressList($val, $this->smtp_params['helo'], null, false);
		                        foreach ($addresses as $address) {
		                            $smtp_recipients[] = sprintf('%s@%s', $address->mailbox, $address->host);
									}
								}
